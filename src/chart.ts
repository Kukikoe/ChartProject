...
class Chart {
    ...
    private data: ForexData | null = null;
    private itemWidth: number = 80;
    private range: number = 0.0004;
    private heightOfOneY: number = 100;
    private candleWidth: number = 50;
    private scrollLeft: number = 0;
    private isDragging: boolean = false;
    private startX: number = 0;
    private perTime: number = 1;
    private length: number = 0;
    private windowWidth: number = window.innerWidth - 16;
    private windowWidthWithoutRightBorder: number;
    private bottomBorder: number = 25;
    private height: number = 500;
    private priceBlockWidth: number = 50;
    private zoomLevel: number = 0.1;
    private zoomFactor: number = 0.1;
    private yRows: number = 1;

    constructor(private dataLink: string) {
    ...
    private processData(dataFromRequest: ForexData) {
    ...
    private handleZoom(e: WheelEvent): void {
        ...
        if ((this.zoomLevel === 0.1 && this.perTime === 1) || this.zoomLevel === 5) return; // Prevent zooming too far out or in
    ...
    private drawChart(): void {
        ...
        const endIndex = Math.min(
            this.length,
            this.perTime * Math.floor((this.scrollLeft + this.windowWidthWithoutRightBorder) / this.itemWidth)
        );
    ...
    private getPriceRange(startIndex: number, endIndex: number): { maxPrice: number, priceRange: number } {
    ...
    private drawCandles(currentPrice: number, startIndex: number, endIndex: number, x: number): void {
        let currentStep = 0;
    ...
    private drawXChartGrid(startIndex: number, endIndex: number, x: number): void {
    ...
    private formatDate(date: Date): string {
        ...
        const hour = parts.find((part: Intl.DateTimeFormatPart) => part.type === 'hour')!.value;
        const minute = parts.find((part: Intl.DateTimeFormatPart) => part.type === 'minute')!.value;

        return `${day} ${month} ${hour}:${minute}`;
    }

    private drawYChartGrid(maxPrice: number): void {
